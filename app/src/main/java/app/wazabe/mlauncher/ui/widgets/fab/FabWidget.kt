package app.wazabe.mlauncher.ui.widgets.fab
    ...
    override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
        ...
        for (appWidgetId in appWidgetIds) {
            ...
            if (fabFlags.getOrNull(0) == true) views.setOnClickPendingIntent(
                R.id.fabPhone,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_PHONE")
            )
            if (fabFlags.getOrNull(1) == true) views.setOnClickPendingIntent(
                R.id.fabMessages,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_MESSAGES")
            )

            if (fabFlags.getOrNull(2) == true) views.setOnClickPendingIntent(
                R.id.fabCamera,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_CAMERA")
            )

            if (fabFlags.getOrNull(3) == true) views.setOnClickPendingIntent(
                R.id.fabPhotos,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_PHOTOS")
            )

            if (fabFlags.getOrNull(4) == true) views.setOnClickPendingIntent(
                R.id.fabBrowser,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_BROWSER")
            )

            if (fabFlags.getOrNull(5) == true) views.setOnClickPendingIntent(
                R.id.fabSettings,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_SETTINGS")
            )
            if (fabFlags.getOrNull(6) == true) views.setOnClickPendingIntent(
                R.id.fabAction,
                getBroadcastPendingIntent(context, "app.wazabe.mlauncher.ui.widgets.fab.FAB_ACTION")
            )
...
class FabClickReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_PHONE" -> context.openDialerApp()
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_MESSAGES" -> context.openTextMessagesApp()
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_CAMERA" -> context.openCameraApp()
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_PHOTOS" -> context.openPhotosApp()
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_BROWSER" -> context.openWebBrowser()
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_SETTINGS" -> context.openDeviceSettings()
            "app.wazabe.mlauncher.ui.widgets.fab.FAB_ACTION" -> AppReloader.startApp(context)
